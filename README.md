...
> - `src/models/*.joblib` — Serialised tuned models generated at runtime by each model script. Gitignored because they are large binary artifacts reproducible from source. A `.gitkeep` placeholder retains the directory structure for first-time clones.
...
For an eldercare early warning system, **Macro F1-Score** is selected as the primary evaluation metric over standard classification accuracy due to inherent class imbalances in smart-home sensor logs. Because elderly residents spend a disproportionate amount of time resting, the dataset is naturally heavily skewed toward "Low Activity" instances. Relying on standard accuracy would reward a naive model that consistently predicts the majority class while failing entirely to catch critical movement transitions. F1-Score mitigates this bias by calculating the precision and recall for each activity class independently and taking their unweighted average. This ensures that "Low", "Moderate", and "High" activity states are treated with equal importance — hence **macro F1** is the metric used for hyperparameter tuning and model comparison across all three models.
